...
def build_graphs(smiles, extra_features_table=None):
    ...
    graphs = []
    for idx, molecule in tqdm(enumerate(molecules), total=len(molecules)):
        if extra_features is not None:
            extra_features_line = extra_features[idx]
        else:
            extra_features_line = None
        graphs.append(mol_2_pytorch_geometric(
                molecule, node_encoder, bond_encoder, extra_features=extra_features_line))
